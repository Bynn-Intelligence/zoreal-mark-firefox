...
async function ensureContent(tabId: number): Promise<boolean> {
  // Only web pages. Probing the extension's own pages, or about: pages, with
  ...
  try {
    ...
    log('tab', tabId, 'cannot be probed:', e instanceof Error ? e.message : String(e)); // about: pages, addons.mozilla.org, and other pages no extension may touch
    return false;
  }
