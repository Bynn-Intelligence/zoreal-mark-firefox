...
**From addons.mozilla.org**, once the listing is live: the link is added here that day.
Firefox installs it on desktop and updates it silently on the next update check after a
new version passes Mozilla's review. Firefox for Android is a separate listing that is
not yet made.

**As a temporary add-on**, from a release, for anyone who wants to run the code they can
read or does not want to wait for the store. Firefox installs a release build
permanently only when Mozilla has signed it, so an unsigned package lasts until Firefox
restarts:

1. Open the [releases page](https://github.com/Bynn-Intelligence/zoreal-mark-firefox/releases)
   and download `zoreal-mark-firefox-<version>.zip` from the latest release. Every tagged
   version has one; CI builds it.
2. Open `about:debugging`, choose **This Firefox**, click **Load Temporary Add-on** and
   pick the zip. The ZOREAL Mark icon appears in the toolbar.
3. After a restart, load it again. Firefox Developer Edition and Nightly can keep an
   unsigned add-on across restarts with `xpinstall.signatures.required` set to false in
   `about:config`; release Firefox cannot.

The same extension for Chrome, Edge, Brave, Vivaldi, Arc and Opera is
[zoreal-mark-chrome](https://github.com/Bynn-Intelligence/zoreal-mark-chrome). Safari is
a separate package.
...
```sh
npm install
npm run dev        # Vite with CRXJS in Firefox mode, hot reload of the extension in dist/
npm run build      # Production build into dist/: talks to the record service only
npm run build:local # The same, pointed at a record service on localhost by default
npm run lint       # web-ext lint over dist/: what addons.mozilla.org checks on upload
npm run run        # web-ext run: a fresh Firefox profile with dist/ loaded, on the demo page
npm run test       # vitest
npm run typecheck  # tsc --noEmit
npm run e2e        # dev/e2e.mjs: the built extension in a real Firefox against the mock
npm run pack       # release/zoreal-mark-firefox-<version>.zip and the source archive
npm run mock       # Local record server serving the conformance fixtures on port 4820
```

Load `dist/` at `about:debugging` (**This Firefox**, **Load Temporary Add-on**, pick
`dist/manifest.json`) and reload it there after every build, or let `npm run run` do it
in a throwaway profile. A build made with `npm run build:local` or `npm run dev` starts
pointed at the mock on `http://localhost:4820`, so nothing is typed into the options
page; a release build starts at the record service and its content security policy
admits nothing else. Open `http://localhost:4820/demo` for a page carrying one Mark per
conformance case, plus one signed for that very page so a strong verdict can be seen.
`/demo-frame` is the same page inside an iframe.

`dev/e2e.mjs` does the same without hands: it starts Firefox over WebDriver BiDi with
`puppeteer-core`, installs `dist/` as a temporary add-on, opens the demo page, and checks
that every fixture case rendered the verdict it expects, that the badge count settles,
and that a strongly verified Mark hides its markers. It uses the Firefox in
`/Applications`; point `FIREFOX` at another binary. Set `E2E_SCREENSHOTS=0` to skip
the screenshots.
...
CI typechecks, tests, builds, lints and packs on every push. Every push to main that
touches the code bumps the patch version, tags it, builds, attaches the package and the
source archive to a GitHub release, and submits both to addons.mozilla.org, where the
version publishes after Mozilla's review. See `.github/workflows/release.yml` for the
secrets it needs and `store/` for the listing's text and assets.

### Layout

```text
src/
  manifest.ts     The Manifest V3 definition CRXJS builds from, in its Firefox shape
...
dev/
  ...
  e2e.mjs         Installs the built extension into Firefox and checks every fixture case
```
